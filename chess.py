...
BOARD_SIZE = 8
SQUARE_SIZE = 80
ANIM_STEPS = 10  # número de frames na animação do movimento
...
class Piece:
    ...
    def moves(self, board, r, c):
        """Retorna lista de (r_dest, c_dest) legais."""
        return []
...
# --- Tabuleiro e Navegação ---
class Board:
    def __init__(self, game):
        self.game = game
        # grid[r][c] = objeto Piece ou None
...
class Game:
    def __init__(self, canvas):
        ...
        self.history = []               # pilha de estados para undo

        self.selected = None            # (r,c) selecionado
        self._draw_board()
        ...
        for r in range(8):
            for c in range(8):
                ...
                self.canvas.create_rectangle(x0,y0,
                    x0+SQUARE_SIZE,y0+SQUARE_SIZE, fill=color, outline="")
    ...
    def on_click(self, event):
        ...
        cur_color = self.turns[0]
        p = self.board.grid[r][c]
        if self.selected is None:
            # selecionar peça própria
            if p and p.color==cur_color:
                self.selected = (r,c)
                self._draw_board()
        else:
            r0,c0 = self.selected
            ...
            if (r,c) in moves:
                # salvar estado p/ undo
                self.history.append((r0,c0,piece, r,c,
                    self.board.grid[r][c], cur_color))
    ...
    def _animate_move(self, r0,c0, r1,c1, piece):
        """Desliza a peça em ANIM_STEPS quadros."""
        img_obj = None
    ...
    def undo(self):
        """Exemplo de operação de pilha (undo)."""
        if not self.history: return
        r0,c0,piece, r1,c1,captured, prev_turn = self.history.pop()
        self.board.grid[r0][c0] = piece
        self.board.grid[r1][c1] = captured
        # reverte fila de turnos
        self.turns.rotate(1)
        self._draw_board()

# --- Análise de Complexidade (exemplo) ---
# A geração de movimentos da Queen, que usa _sliding, visita no máximo 7 casas
# em cada uma das 8 direções: O(1) * 8 * 7 = O(1) (constante),
# mas em termos de n peças no tabuleiro, somar todos os pieces.moves() custaria O(n).
